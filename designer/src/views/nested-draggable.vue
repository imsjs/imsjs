<template>
  ...
  <VueDraggable :class="prefixCls" group="components" tag="div" v-model="list">
    ...
    <div
      ...
      @click.stop="onActiveItem(element)"
    >
...
// const modelCheckedItem = defineModel<any>("checkedItem", { default: {} });
...
const onActiveItem = (item: any) => {
  ...
  // console.info('activeItem =>',activeItem.value)

  // activeItem.value = {};
  
  activeItem.value = item;

  // modelCheckedItem.value = item;

  // console.info("activeItem.value =>", activeItem.value.title,activeItem.value.id);
};
</script>
<style scoped>
